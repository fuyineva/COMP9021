...
def leftmost_longest_path_from_top_left_corner():
    ...
    if not grid[0][0]:
        return []
    paths = Queue()
    paths.enqueue(([(0, 0)], '')) #enqueue的是tuple([LIST],previous_direction)
    #print('Path',paths)
    #
    while not paths.is_empty():
        (path, previous_direction) = paths.dequeue()
        #path is a list of typles
        ...
        for new_direction in next_directions[previous_direction]:
            ...
            print('newposition',new_position)
            ...
            if not grid[new_position[0]][new_position[1]]:
                continue
            path_copy = list(path)
            path_copy.append(new_position) #给path加上新的tuple(position),
            paths.enqueue((path_copy, new_direction)) #再把path enqueue回去
            #print('Path_after',paths)
    return path
