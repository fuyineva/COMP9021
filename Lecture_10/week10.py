...
class Tree:
    ...
    def is_bst(self):
        ...
        if self.left_subtree.value is not None:
            ...
            if largest_smaller_tree.value >=self.value:
                return False
        if self.right_subtree.value is not None:
            largest_smaller_tree = self.right_subtree
            while largest_smaller_tree.left_subtree.value is not None:
                largest_smaller_tree = largest_smaller_tree.left_subtree
            if largest_smaller_tree.value <=self.value:
                return False
